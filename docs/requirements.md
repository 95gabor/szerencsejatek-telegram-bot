...
`<code>` for **numeric** values; show **heti nyeremények** (if available) and **találat** count plus
**egyező (növ.)** as spaced `<code>` blocks; full szelvény row still highlights matches with
`<b>...</b>` around `<code>`; do **not** wrap **slash-commands** in `<code>` (product rule).
**`/result`** uses the same formatter as post-draw notifications (plus source line).
...
| FR-3  | `BetHuOtoslottoFetcher` (winning numbers + weekly prize amounts when available), `handle_draw_update_requested`, `handle_draw_result_persist`, `DrizzleDrawRecordRepository`, `src/server.ts`; `/result` → `formatOtoslottoUserMessage` in `register_handlers` |
...
- **`/result`:** latest stored draw for `GAME_ID`; same heti nyeremény + per-line találat + egyező
  (növ.) + szelvény formatting as draw notifications, plus **forrás** line (§7).
...
Telegram/HTML/i18n, weekly prize amounts in notifications and `/result`, NFR-5; align with pipeline
and `docs/adr/`._
